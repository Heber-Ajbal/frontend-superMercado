...
import ActualizarProducto from '~/api/productos/actualizarProducto.gql'
...
watch(() => props.producto, (nuevo) => {
  form.value = nuevo
    ? {
        id: nuevo.codProducto, // Capturamos el ID para edición
        nombre: nuevo.nombre,
    ...
    : { id: null, nombre: '', precioCompra: 0, precioVenta: 0, categoriaId: 1, imagen: null }
}, { immediate: true })
...
const { mutate: crearProducto } = useMutation(CrearProducto)
const { mutate: actualizarProducto } = useMutation(ActualizarProducto)
  ...
  try {
    if (form.value.id !== null) {
      ...
      await actualizarProducto({
        input: {
          idProducto: form.value.id, // Enviamos ID correctamente
      ...
      <form @submit.prevent="guardar" class="space-y-4">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1 flex gap-2">
            ...
            <button type="button" class="bg-blue-600 text-white px-2 rounded text-sm" @click="showCategoriaModal = true">+</button>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-2 pt-4">
          <button type="button" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600" @click="$emit('close')">Cancelar</button>
